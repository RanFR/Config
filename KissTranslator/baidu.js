...
(text, from, to, url, key) => {
  ...
  // 解析appid和secretKey，使用#分割
  const [appid, secretKey] = key.split("#");
  if (!appid || !secretKey) {
    throw new Error("密钥格式错误，请使用格式：appid#secretKey");
  }

  // 生成随机数salt
  const salt = Date.now().toString();

  // 构建签名字符串：appid + query + salt + secretKey
...
(res, text, from, to) => {
  ...
  if (data.trans_result && data.trans_result.length > 0) {
    ...
    const detectedFrom = data.from || from;

    // 判断检测到的语言是否与目标语言相同
    const isSameLanguage = detectedFrom === to;

    return [translatedText, isSameLanguage];
  }
