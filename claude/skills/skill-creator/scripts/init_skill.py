...
SKILL_TEMPLATE = """---
name: {skill_name}
description: [TODO: 完整且信息丰富的解释，说明技能的作用以及何时使用。包括何时使用此技能 - 触发它的特定场景、文件类型或任务。]
---
...
**注意：**脚本可以在不加载到上下文的情况下执行，但仍可由 Claude 读取以进行补丁或环境调整。

### references/
意在加载到上下文中以指导 Claude 流程和思维的文档和参考资料。
...
**适用于：**深度文档、API 参考、数据库架构、综合指南或 Claude 在工作时应参考的任何详细信息。
...
**适用于：**模板、样板代码、文档模板、图像、图标、字体或意在最终输出中复制或使用的任何文件。
...
资产文件不意在加载到上下文中，而是在 Claude 产生的输出中使用。
